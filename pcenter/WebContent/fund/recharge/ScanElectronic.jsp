                        ...
                        <li>
                            <div class="bankinfo bankinfo-m">
                                ...
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <span class="orange paidname select" data-clipboard-text="${firstPayAccount.account}" name="copy">
                                            <em class="bank-number" ${thirdBankCode != 'onecodepay'?'id="bankNum"':''}>${firstPayAccount.account}</em>
    ...
    <div class="left-ico-message">
        ...
        <c:if test="${thirdBankCode eq 'alipay'}">
            <div class="control-group" name="electronicElement" style="${firstPayAccount.type eq '1'?'':'display:none'}">
                <label class="control-label" for="result.payerName">${views.fund_auto['您的支付宝昵称']}：</label>
                <div class="controls">
                    <input style="width: 200px" type="text" class="input" id="result.payerName" name="result.payerName" placeholder="${views.fund_auto['请填入您的支付宝昵称']}">
                </div>
            </div>
        </c:if>
            ...
            <div class="controls">
                <c:choose>
                    <c:when test="${siteId == '119' || siteId == '270'}">
                        <input  id="merchantNumber" style="width: 200px" type="text" class="input" maxlength="5"  placeholder="${views.fund_auto['请填写商户订单号']}${views.fund_auto['必填']}" maxlength="5" name="result.bankOrder" autocomplete="off">
                        <span id="merchantNumberPrompt" class="orange merchant-number-prompt"><i class="mark plaintsmall"></i>${views.fund_auto['商户订单号不能为空']}</span>
                    </c:when>
                    <c:otherwise><%--style="visibility:hidden"--%>
