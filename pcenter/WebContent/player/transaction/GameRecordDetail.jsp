...
<div class="notice">
    ...
    <div class="return" style="margin-top: 1px">
        <a href="/vPlayerGameOrder/singleApiLog.html?search.apiId=${command.search.apiId}&search.gameType=${command.search.gameType}" class="btn btn-gray btn-big" nav-target="mainFrame">${views.player_auto['返回上一级']}</a>
    </div>
</div>
    ...
    <div class="record-detail">
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="">
            <tr>
                ...
                <td width="80%" class="arial">${command.gameRecordDetail['username']}</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td class="arial">${soulFn:formatDateTz(command.gameRecordDetail['bettime'],DateFormat.DAY_SECOND ,timeZone )}</td>
            </tr>
            <tr>
                <td class="datailname bold">${views.bet['History.view.transaction']}：</td>
                <td class="arial">${command.gameRecordDetail['singleamount']}</td>
            </tr>
            <%--<tr>
              <td class="datailname bold">${views.bet['History.view.payOutTime']}：</td>
              <td class="arial">${soulFn:formatDateTz(command.gameRecordDetail['payouttime'],DateFormat.DAY_SECOND ,timeZone )}</td>
            </tr>--%>
            <tr>
                <td class="datailname bold">${views.bet['History.view.payOut']}：</td>
                <td class="arial"><span class="orange">${empty command.gameRecordDetail['profitamount']?'-':command.gameRecordDetail['profitamount']}</span></td>
            </tr>
            <c:if test="${! empty command.gameRecordDetail['winningamount']}">
                <tr>
                    <td class="datailname bold">${views.player_auto['彩池奖金']}：</td>
                    <td class="arial"><span class="orange">${command.gameRecordDetail['winningamount']}</span></td>
                </tr>
            </c:if>
            <tr>
                <td class="datailname bold">${views.bet['History.view.effectiveTransaction']}：</td>
                <td class="arial"><span class="blue">${empty command.gameRecordDetail['effectamounts']?'-':command.gameRecordDetail['effectamounts']}</span></td>
            </tr>
            <tr>
                    ...
                    <c:forEach items="${resultArray}" var="array">
                        <table width="90%" border="0" cellspacing="0" cellpadding="0">
                            <c:forEach items="${array}" var="json">
                                <c:if test="${!empty views.gameOrder[json.key]||json.key=='type'}">
                                    <tr>
                                        <td class="datailname datailback">
                                            <c:choose>
                                                <%--MG金额类型--%>
                                        ...
                                        <td class="arial">
                                            <c:choose>
                                                ...
                                                <c:when test="${json.key=='liveMemberReportDetails'}">
                                                    <c:set var="value" value="${json.value.get(0)['betType']}"/>
                                                ...
                                                <c:when test="${json.key=='resultList'}">
                                                    ...
                                                    <c:if test="${array['gameType']=='BULL_BULL'}">
                                                        ...
                                                        ${views.player_auto['庄']}（${views.player_auto['点数']}【${views.gameOrder[key]}】，${views.player_auto['最大牌面']} 【
                                                        ...
                                                        <c:if test="${(maxValue/13).intValue()==3}">
                                                        ...
                                                        <c:forEach var="i" begin="1" end="3" varStatus="v">
                                                            ...
                                                            ${views.player_auto['闲']}${v.index}(${views.player_auto['点数']}【${views.gameOrder[key]}】,
                                                            ${views.player_auto['最大牌面']}【
                                                            ...
                                                            ${views.player_auto['结果']}【${views.gameOrder[key]}】)
                                                        </c:forEach>
                                                    </c:if>
                                                </c:when>
                                                <%--DS结果牌--%>
                                                <c:when test="${json.key=='pokerList'}">
                                                    ...
                                                    <c:if test="${array['gameType']=='DRAGON_TIGER'}">
                                                        <c:forEach var="value" items="${json.value}" varStatus="vs">
                                                            <c:if test="${vs.index==0}">${views.player_auto['龙']}：</c:if>
                                                            <c:if test="${vs.index==1}">${views.player_auto['虎']}：</c:if>
                                                            <c:set var="round" value="${value%13==0?13:value%13}"/>
                                                            <c:if test="${(value/13).intValue()==0}">
                                                                <em class="gr gr-spade-${round} fs1"></em>
                                                            </c:if>
                                                    ...
                                                    <c:if test="${array['gameType']=='BULL_BULL'}">
                                                        ...
                                                        <c:forEach begin="1" end="4" varStatus="v">
                                                            ...
                                                            <c:if test="${v.index==1}">${views.player_auto['庄']}&nbsp;（</c:if>
                                                            <c:if test="${v.index!=1}">${views.player_auto['闲']}${v.index-1}（</c:if>
                                                ...
                                                <%--OG投注类型、游戏结果、玩家投注--%>
                                                <c:when test="${json.key=='GameKind'||json.key=='ResultType'||json.key=='GameBettingKind'}">
                                                    <c:set var="key" value="${json.key}.${json.value}"/>
                                                    ${views.gameOrder[key]}
                                                </c:when>
                                                <%--SLC投注详细--%>
                                                ...
                                                <c:when test="${json.key=='Card'&&array['GameType']=='3006'}">
                                                    <c:forEach items="${fn:split(json.value, '*')}" var="value" varStatus="vs">
                                                        <c:if test="${vs.index==0}">
                                                            ${views.player_auto['庄家']}：（
                                                        </c:if>
                                                        <c:if test="${vs.index==1}">
                                                            ${views.player_auto['顺门']}：（
                                                        </c:if>
                                                        <c:if test="${vs.index==2}">
                                                            ${views.player_auto['出门']}：（
                                                        </c:if>
                                                        <c:if test="${vs.index==2}">
                                                            ${views.player_auto['到门']}：（
                                                        </c:if>
                                                        <c:forEach items="${fn:split(value, ',')}" var="i">
                                                            <c:if test="${fn:contains(i, 'S')}">
                                                                <em class="gr gr-spade-${fn:split(i, '.')[1]} fs1"></em>
                                                ...
                                                <c:when test="${json.key=='Card'&&array['GameType']=='3010'}">
                                                    <c:forEach items="${fn:split(json.value, '*')}" var="value" varStatus="vs">
                                                        ...
                                                        <c:if test="${vs.index==1}">
                                                            ${views.player_auto['闲家']}：（
                                                        </c:if>
                                                        <c:if test="${vs.index==2}">
                                                            ${views.player_auto['公牌']}：（
                                                        </c:if>
                                                        <c:forEach items="${fn:split(value, ',')}" var="i">
                                                            <c:if test="${fn:contains(i, 'S')}">
                                                                <em class="gr gr-spade-${fn:split(i, '.')[1]} fs1"></em>
                                                ...
                                                <c:when test="${json.key=='Card'&&(array['GameType']=='3005'||array['GameType']=='3012')}">
                                                    <c:forEach items="${fn:split(json.value, '*')}" var="value" varStatus="vs">
                                                        ...
                                                        <c:if test="${vs.index==1}">
                                                            ${views.player_auto['闲一']}：（
                                                        </c:if>
                                                        <c:if test="${vs.index==2}">
                                                            ${views.player_auto['闲二']}：（
                                                        </c:if>
                                                        <c:if test="${vs.index==3}">
                                                            ${views.player_auto['闲三']}：（
                                                        </c:if>
                                                        <c:forEach items="${fn:split(value, ',')}" var="i">
                                                            <c:if test="${fn:contains(i, 'S')}">
                                                                <em class="gr gr-spade-${fn:split(i, '.')[1]} fs1"></em>
                                                ...
                                                <c:otherwise>
                                                    ${empty json.value||json.value==null?'':json.value}
                                                </c:otherwise>
                                            </c:choose>
                                        </td>
                                    </tr>
